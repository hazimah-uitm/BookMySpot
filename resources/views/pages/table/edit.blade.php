        ...
        <form method="POST" action="{{ $save_route }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control {{ $errors->has('table_no') ? 'is-invalid' : '' }}" id="table_no"
                    name="table_no" value="{{ old('table_no', $table->table_no ?? '') }}">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control {{ $errors->has('total_seat') ? 'is-invalid' : '' }}" id="total_seat"
                    name="total_seat" value="{{ old('total_seat', $table->total_seat ?? '') }}">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control {{ $errors->has('available_seat') ? 'is-invalid' : '' }}" id="available_seat"
                    name="available_seat" value="{{ old('available_seat', $table->available_seat ?? '') }}">
            ...
            <div class="mb-3">
                ...
                <div class="form-check">
                    <input type="radio" id="Tersedia" name="status" value="Tersedia"
                        {{ old('status', $table->status ?? '') == 'Tersedia' ? 'checked' : '' }}
                        required>
                    <label class="form-check-label" for="Tersedia">Tersedia</label>
                </div>
                <div class="form-check">
                    <input type="radio" id="Penuh" name="status" value="Penuh"
                        {{ old('status', $table->status ?? '') == 'Penuh' ? 'checked' : '' }}
                        required>
